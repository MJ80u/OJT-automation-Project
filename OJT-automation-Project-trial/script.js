...
function populateYearSelector() {
    ...
    const years = [];
    for (let y = thisYear; y <= 2030; y++) years.push(y);
...
function deleteRow(index) {
    if (currentEditablePrograms.length <= 1) {
        showToast('You must have at least one program row.', 'warning');
        return;
    }
    showConfirmDialog({
        title: 'Delete Row',
...
function loadReport(skipInit) {
    ...
    const rows = currentEditablePrograms.map((program, index) => `
        <tr>
            ...
            <td><input type="number" id="ab_ps_${index}" min="0" step="0.01" value="${program.annualBudget?.ps || 0}" oninput="saveBudget(${index});calculate(${index})" aria-label="Annual Budget PS" class="input-number" readonly></td>
            <td><input type="number" id="ab_mooe_${index}" min="0" step="0.01" value="${program.annualBudget?.mooe || 0}" oninput="saveBudget(${index});calculate(${index})" aria-label="Annual Budget MOOE" class="input-number" readonly></td>
            <td><input type="number" id="ab_co_${index}" min="0" step="0.01" value="${program.annualBudget?.co || 0}" oninput="saveBudget(${index});calculate(${index})" aria-label="Annual Budget CO" class="input-number" readonly></td>
            <td><input type="number" id="ab_total_${index}" min="0" step="0.01" value="${program.annualBudget?.total || 0}" oninput="saveBudget(${index});calculate(${index})" aria-label="Annual Budget Total" class="input-number" readonly></td>
            ...
            <td>${isAipMode ? '' : '<button onclick="deleteRow(' + index + ')" class="delete-btn" title="Delete this row">✕</button>'}</td>
        </tr>
    `).join('');
    ...
    document.getElementById('report-container').innerHTML = `
        <div class="report-toolbar">
            ...
            <div>
                <button onclick="addRow()" class="btn btn-outline btn-sm">+ Add Row</button>
...
function calculatePhysical(index) {
    ...
    if (isNaN(targetNum) || isNaN(actualNum) || targetVal === '' || actualVal === '') {
        document.getElementById(`physical_variance_${index}`).textContent = 'N/A';
        document.getElementById(`physical_accomplishment_${index}`).textContent = 'N/A';
    } else {
        const variance = targetNum - actualNum;
        const accomplishment = targetNum > 0 ? (actualNum / targetNum) * 100 : 0;
        document.getElementById(`physical_variance_${index}`).textContent = formatCurrency(variance);
        document.getElementById(`physical_accomplishment_${index}`).textContent = `${accomplishment.toFixed(2)}%`;
    }
}
...
function loadPhysicalPerformance(skipInit) {
    ...
    const rows = currentEditablePrograms.map((program, index) => `
        <tr>
            ...
            <td><span id="physical_variance_${index}">0.00</span></td>
            <td><span id="physical_accomplishment_${index}">0.00%</span></td>
            <td><input type="text" id="physical_remarks_${index}" aria-label="Physical remarks" class="input-text"></td>
            <td>${isAipMode ? '' : '<button onclick="deleteRow(' + index + ')" class="delete-btn" title="Delete this row">✕</button>'}</td>
        </tr>
    `).join('');
    const actionColspan = isAipMode ? 0 : 1;
    const pTotalRow = `<tr class="total-row">
        ...
        <td colspan="${16 - actionColspan}"></td>
    </tr>`;
...
function loadAnnualFinancialPerformance(skipInit) {
    ...
    const rows = currentEditablePrograms.map((program, index) => `
        <tr>
            ...
            <td><input type="number" id="aip_mooe_${index}" min="0" step="0.01" value="${program.aipAmount?.mooe || 0}" aria-label="AIP MOOE" class="input-number" readonly></td>
            <td><input type="number" id="aip_co_${index}" min="0" step="0.01" value="${program.aipAmount?.co || 0}" aria-label="AIP CO" class="input-number" readonly></td>
            <td><input type="number" id="ab_ps_${index}" min="0" step="0.01" value="${program.annualBudget?.ps || 0}" oninput="saveBudget(${index});calculateAnnualFinancial(${index})" aria-label="Annual Budget PS" class="input-number" readonly></td>
            <td><input type="number" id="ab_mooe_${index}" min="0" step="0.01" value="${program.annualBudget?.mooe || 0}" oninput="saveBudget(${index});calculateAnnualFinancial(${index})" aria-label="Annual Budget MOOE" class="input-number" readonly></td>
            <td><input type="number" id="ab_co_${index}" min="0" step="0.01" value="${program.annualBudget?.co || 0}" oninput="saveBudget(${index});calculateAnnualFinancial(${index})" aria-label="Annual Budget CO" class="input-number" readonly></td>
            <td><input type="number" id="ab_total_${index}" min="0" step="0.01" value="${program.annualBudget?.total || 0}" oninput="saveBudget(${index});calculateAnnualFinancial(${index})" aria-label="Annual Budget Total" class="input-number" readonly></td>
...
function loadAip(skipInit) {
    ...
    if (!skipInit) {
        currentAipRows = Array.from({ length: 10 }, () => createEmptyAipRow());
    }
    ...
    const rows = currentAipRows.map((row, index) => `
        <tr>
            ...
            <td><input type="date" id="aip_start_${index}" value="${escapeHtml(row.startDate)}" class="input-text"></td>
            <td><input type="date" id="aip_end_${index}" value="${escapeHtml(row.completionDate)}" class="input-text"></td>
...
function submitAip(btn) {
    ...
    const hasAnyEntry = rows.some(r => r.refCode || r.program || r.ps > 0 || r.mooe > 0 || r.fe > 0 || r.co > 0);
    if (!hasAnyEntry) {
...
function renderOfficesTable() {
    ...
    .then(([serverOffices, deadlineData, submissionsData]) => {
        ...
        if (deadlineStr) {
            ...
            deadlineInfo = { date: deadlineDate, daysRemaining: diffDays, isOverdue: diffDays < 0 };
        }
        ...
        if (deadlineInfo) {
            ...
            if (deadlineInfo.isOverdue) {
                const overdueBy = Math.abs(deadlineInfo.daysRemaining);
                banner.style.background = '#f8d7da';
                banner.style.color = '#721c24';
                banner.innerHTML = `Deadline was ${formatDateTime(deadlineStr)} — Overdue by ${overdueBy} day${overdueBy !== 1 ? 's' : ''}`;
            } else if (deadlineInfo.daysRemaining <= 7) {
                banner.style.background = '#fff3cd';
                banner.style.color = '#856404';
                banner.innerHTML = `Deadline: ${formatDateTime(deadlineStr)} — ${deadlineInfo.daysRemaining} day${deadlineInfo.daysRemaining !== 1 ? 's' : ''} remaining`;
            } else {
        ...
        serverOffices.forEach(office => {
            ...
            if (deadlineInfo) {
                ...
                if (deadlineInfo.isOverdue) {
                    ...
                    badgeText = deadlineInfo.daysRemaining === 0 ? 'Due today' : `${deadlineInfo.daysRemaining}d left`;
                } else {
                    badgeClass = 'deadline-far';
                    badgeText = `${deadlineInfo.daysRemaining}d left`;
                }
...
function loadEmployeeDeadline() {
    ...
    .then(data => {
        ...
        if (diffDays < 0) {
            ...
            el.textContent = `Deadline was ${formatDateTime(data.global)} — Overdue by ${Math.abs(diffDays)} day${Math.abs(diffDays) !== 1 ? 's' : ''}`;
        } else if (diffDays <= 7) {
            el.style.background = '#fff3cd'; el.style.color = '#856404';
            el.textContent = `Deadline: ${formatDateTime(data.global)} — ${diffDays} day${diffDays !== 1 ? 's' : ''} remaining`;
        } else {
...
function renderQuarterlyDetail(programs, office, headEl, bodyEl, readOnly) {
    ...
    const bodyRows = programs.map((p, i) => {
        ...
        <tr>
            ...
            <td><button class="btn btn-outline btn-sm" style="color:#e74c3c;border-color:#e74c3c;padding:2px 6px;" onclick="confirmDeleteRow(this)">×</button></td>
        </tr>`;
    }).join('');
    ...
    const bodyRows = programs.map((p, i) => {
        if (readOnly) {
            ...
            <tr>
                ...
                <td><span>${fmtV(p.variance)}</span></td>
                <td><span>${fmtA(p.accomplishment)}</span></td>
                <td><span>${escapeHtml(p.remarks || '')}</span></td>
            </tr>`;
        }
        return `
        <tr>
            ...
            <td><input type="text" value="${p.targetTotal || ''}" data-idx="${i}" data-field="target_total" style="width:100px"></td>
            ...
            <td><input type="text" value="${p.actualTotal || ''}" data-idx="${i}" data-field="actual_total" style="width:100px"></td>
            <td><span id="v_${i}">${p.variance !== undefined && p.variance !== 'N/A' ? formatCurrency(p.variance) : (p.variance || 'N/A')}</span></td>
            <td><span id="abs_${i}">${p.accomplishment !== undefined && p.accomplishment !== 'N/A' ? p.accomplishment + '%' : (p.accomplishment || 'N/A')}</span></td>
            <td><input type="text" value="${escapeHtml(p.remarks || '')}" data-idx="${i}" data-field="remarks" style="width:150px"></td>
            <td><button class="btn btn-outline btn-sm" style="color:#e74c3c;border-color:#e74c3c;padding:2px 6px;" onclick="confirmDeleteRow(this)">×</button></td>
        </tr>`;
    }).join('');
...
function renderAipOfficeEditorDetail(rows, headEl, bodyEl) {
    ...
    bodyEl.innerHTML = rows.map((r, i) => {
        ...
        <tr>
            ...
            <td><input type="date" value="${escapeHtml(r.startDate || '')}" data-idx="${i}" data-field="aip_start" style="width:140px"></td>
            <td><input type="date" value="${escapeHtml(r.completionDate || '')}" data-idx="${i}" data-field="aip_end" style="width:140px"></td>
...
function addReportRow() {
    ...
    if (type === 'physical') {
        ...
        emptyRow = `<tr>
            ...
            <td><input type="date" value="" data-idx="${idx}" data-field="aip_start" style="width:140px"></td>
            <td><input type="date" value="" data-idx="${idx}" data-field="aip_end" style="width:140px"></td>
...
function addOffice(btn) {
     const id = prompt('Enter Office ID (e.g. NEW-OFFICE):');
     ...
     fetchWithTimeout('/api/offices', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify({ id: id.trim(), name: name.trim(), password: password.trim() })
     })
     .then(r => r.json())
     .then(d => {
         if (d.success) {
...
function deleteOffice(officeId) {
    showConfirmDialog({
        ...
        .then(d => {
            if (d.success) {
                refreshOfficeList().then(() => renderOfficesTable());
            } else {
...
function logout() {
     ...
     // Refresh office list when returning to login
     refreshOfficeList().then(() => populateOfficeSelect()).catch(() => populateOfficeSelect());
    document.getElementById('report-container').innerHTML = '';
