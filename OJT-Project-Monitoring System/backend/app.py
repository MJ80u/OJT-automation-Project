...
FRONTEND_DIR = os.path.join(_BASEDIR, '..', 'frontend')
...
def require_auth(office_id=None):
...
def init_db():
    if not os.path.exists(DB_PATH):
        _localappdata = os.path.join(
            os.environ.get('LOCALAPPDATA', os.path.expanduser('~\\AppData\\Local')),
            'OJT-automation', 'database.db'
        )
        if os.path.exists(_localappdata):
            try:
                os.makedirs(os.path.dirname(DB_PATH), exist_ok=True)
                import shutil
                shutil.copy2(_localappdata, DB_PATH)
            except Exception:
                pass
    conn = get_db()
    conn.execute('''
...
def seed_programs():
    ...
    try:
        with open(os.path.join(FRONTEND_DIR, 'js', 'office-data.generated.js'), 'r') as f:
            content = f.read()
...
def submit_report(): 
    ...
    if not office_id or not body.get('submittedAt'):
        return jsonify({'success': False, 'error': 'invalid_input', 'message': 'Missing required fields.'}), 400
    
    if not require_auth(office_id):
