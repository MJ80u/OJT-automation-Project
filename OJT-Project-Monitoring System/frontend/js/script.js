        ...
        .then(data => {
            if (data && data.rows && data.rows.length > 0) {
                ...
                return data.rows.map(row => {
                    const existing = office?.programs?.find(p => p.code === row.refCode || p.name === row.program);
                    return {
                        code: row.refCode || '',
                        name: row.program || '',
    ...
    .then(data => {
        ...
        if (type === 'quarterly') {
            ...
            submitted.forEach((prog, idx) => {
                ...
                const varEl = document.getElementById(`physical_variance_${idx}`);
                const accEl = document.getElementById(`physical_accomplishment_${idx}`);
                if (varEl) varEl.value = prog.variance !== undefined && prog.variance !== null && prog.variance !== 'N/A' ? prog.variance : 'N/A';
                if (accEl) accEl.value = prog.accomplishment !== undefined && prog.accomplishment !== null && prog.accomplishment !== 'N/A' ? prog.accomplishment : 'N/A';
            });
            ...
            submitted.forEach((prog, idx) => {
                ...
                if (prog.annualBudget && (prog.annualBudget.ps || prog.annualBudget.mooe || prog.annualBudget.co)) {
                    ...
                    if (totalEl) totalEl.value = (prog.annualBudget.ps || 0) + (prog.annualBudget.mooe || 0) + (prog.annualBudget.co || 0);
                }
...
function loadPhysicalPerformance(skipInit) {
    ...
    const rows = currentEditablePrograms.map((program, index) => `
        <tr>
            ...
            <td><input type="text" id="target_total_${index}" aria-label="Target total" class="input-number"></td>
            ...
            <td><input type="text" id="actual_total_${index}" aria-label="Actual total" class="input-number"></td>
            <td><input type="text" id="physical_variance_${index}" class="input-number" style="width:100px"></td>
            <td><input type="text" id="physical_accomplishment_${index}" class="input-number" style="width:100px"></td>
...
function applyQuarterlyObligationsToAdminTable(officeId, bodyEl) {
    if (!officeId || !bodyEl) return;
    const promises = ['Q1', 'Q2', 'Q3', 'Q4'].map(q =>
        fetch(`/api/reports/${officeId}?quarter=${q}&type=quarterly&year=${currentYear}`)
            .then(r => r.ok ? r.json() : null)
    );
    ...
    Promise.all(promises).then(results => {
        ...
        const rows = bodyEl.querySelectorAll('tr:not(.total-row)');
        rows.forEach(row => {
            const codeInput = row.querySelector('[data-field="code"]');
            const obligEl = row.querySelector('[data-field="obligations"]');
            if (codeInput && obligEl && totalsByCode[codeInput.value] !== undefined) {
                const val = totalsByCode[codeInput.value];
                if (obligEl.value !== undefined) obligEl.value = val;
                else obligEl.textContent = val.toFixed(2);
                autoComputeRow(obligEl);
            }
        });
    }).catch(() => {});
}
...
function loadAnnualFinancialPerformance(skipInit) {
    ...
    if (!skipInit) {
        ...
        tryLoadAipRows(office.id).then(aipRows => {
            if (aipRows) {
                ...
                currentEditablePrograms = aipRows;
                if (office?.programs) {
                    currentEditablePrograms = currentEditablePrograms.map(prog => {
                        const match = office.programs.find(p => p.code === prog.code || p.name === prog.name);
                        if (match?.annualBudget) {
                            return { ...prog, annualBudget: match.annualBudget };
                        }
                        return prog;
                    });
                }
    ...
    const aTot = currentEditablePrograms.reduce((t, p) => {
        ...
        const ps = ab.ps || 0, mooe = ab.mooe || 0, co = ab.co || 0;
        t.aipPs += aip.ps || 0; t.aipMooe += aip.mooe || 0; t.aipCo += aip.co || 0;
        t.abPs += ps; t.abMooe += mooe; t.abCo += co; t.abTotal += ps + mooe + co;
        t.allot += ps + mooe + co;
        return t;
    }, { aipPs: 0, aipMooe: 0, aipCo: 0, abPs: 0, abMooe: 0, abCo: 0, abTotal: 0, allot: 0 });
...
function submitPhysicalPerformance(btn) {
    ...
    const results = updatedPrograms.map((program, index) => {
        ...
        const rawVar = document.getElementById(`physical_variance_${index}`).value.trim();
        const rawAcc = document.getElementById(`physical_accomplishment_${index}`).value.trim();
        const varNum = parseFloat(rawVar.replace(/,/g, ''));
        const accNum = parseFloat(rawAcc.replace(/%/g, '').replace(/,/g, ''));
        ...
        return {
            ...
            actual,
            actualTotal,
            variance: isNaN(varNum) ? (rawVar || 'N/A') : varNum,
            accomplishment: isNaN(accNum) ? (rawAcc || 'N/A') : accNum,
    ...
    .then(data => {
        if (type === 'aip') {
            ...
            if (type === 'physical') {
                ...
                if (type === 'annual_financial') {
                    applyQuarterlyObligationsToAdminTable(viewingReportOfficeId, document.getElementById('report-detail-body'));
                }
            }
        }
    })
...
function renderPhysicalDetail(programs, office, headEl, bodyEl, readOnly) {
    ...
    const bodyRows = programs.map((p, i) => {
        ...
        <tr>
            ...
            <td><input type="text" value="${p.variance !== undefined && p.variance !== 'N/A' ? p.variance : (p.variance || '')}" data-idx="${i}" data-field="variance" style="width:100px"></td>
            <td><input type="text" value="${p.accomplishment !== undefined && p.accomplishment !== 'N/A' ? p.accomplishment : (p.accomplishment || '')}" data-idx="${i}" data-field="accomplishment" style="width:100px"></td>
            <td><input type="text" value="${escapeHtml(p.remarks || '')}" data-idx="${i}" data-field="remarks" style="width:150px"></td>
            <td><button class="btn btn-outline btn-sm" style="color:#e74c3c;border-color:#e74c3c;padding:2px 6px;" onclick="deleteAdminRow(this)">×</button></td>
        </tr>`;
    }).join('');
...
function autoComputeRow(el) {
    ...
    const allotmentSpan = row.querySelector('[data-field="allotment"]');
    if (allotmentSpan) allotmentSpan.textContent = allotment.toFixed(2);
    const obligEl = row.querySelector('[data-field="obligations"]');
    const obligations = parseFloat(obligEl?.value ?? obligEl?.textContent) || 0;
    const variance = allotment - obligations;
    const absorptive = allotment > 0 ? (obligations / allotment) * 100 : 0;
...
function renderAnnualDetail(programs, office, headEl, bodyEl, readOnly) {
    ...
    programs = programs.filter(p => p.name !== 'TOTAL APPROPRIATION' && p.name !== 'TOTAL APPROPRIATIONS');
    const bodyRows = programs.map((p, i) => {
        const master = progData.find(m => m.code === p.code || m.name === p.name) || {};
        const hasAip = p.aipAmount && (p.aipAmount.ps || p.aipAmount.mooe || p.aipAmount.co);
        const hasBudget = p.annualBudget && (p.annualBudget.ps || p.annualBudget.mooe || p.annualBudget.co);
        const aip = hasAip ? p.aipAmount : (master.aipAmount || p.aipAmount || {});
        const ab = hasBudget ? p.annualBudget : (master.annualBudget || p.annualBudget || {});
        const fmt = v => (v || 0).toFixed(2);
        if (readOnly) {
            return `
            <tr>
        ...
        <tr>
            ...
            <td><input type="number" step="0.01" value="${ab.co || 0}" data-idx="${i}" data-field="ab_co" style="width:110px" oninput="autoComputeRow(this)"></td>
            <td><input type="number" step="0.01" value="${ab.total || 0}" data-idx="${i}" data-field="ab_total" style="width:110px"></td>
            <td><span data-idx="${i}" data-field="allotment">${(p.allotment || ((ab.ps||0)+(ab.mooe||0)+(ab.co||0))).toFixed(2)}</span></td>
            <td><span data-idx="${i}" data-field="obligations">${(p.obligations || 0).toFixed(2)}</span></td>
            <td><span id="v_${i}" data-display="variance">${(p.variance || 0).toFixed(2)}</span></td>
            <td><span id="abs_${i}" data-display="absorptive">${(p.absorptive || 0).toFixed(2)}%</span></td>
            <td><input type="text" value="${escapeHtml(p.remarks || '')}" data-idx="${i}" data-field="remarks" style="width:150px"></td>
    ...
    programs.forEach(p => {
        ...
        const ab = hasBudget ? p.annualBudget : (master.annualBudget || p.annualBudget || {});
        tot.aipPs += aip.ps || 0; tot.aipMooe += aip.mooe || 0; tot.aipCo += aip.co || 0;
        tot.abPs += ab.ps || 0; tot.abMooe += ab.mooe || 0; tot.abCo += ab.co || 0; tot.abTotal += (ab.ps || 0) + (ab.mooe || 0) + (ab.co || 0);
    ...
    if (type === 'physical') {
        emptyRow = `
            <tr>
                ...
                <td><input type="text" value="" data-idx="${idx}" data-field="variance" style="width:100px"></td>
                <td><input type="text" value="" data-idx="${idx}" data-field="accomplishment" style="width:100px"></td>
        ...
        emptyRow = `
            <tr>
                ...
                <td><span data-idx="${idx}" data-field="allotment">0.00</span></td>
                <td>${isAnnual ? '<span data-idx="' + idx + '" data-field="obligations">0.00</span>' : '<input type="number" step="0.01" value="0" data-idx="' + idx + '" data-field="obligations" style="width:130px">'}</td>
...
function saveReportDetail(btn) {
    ...
    rows.forEach((tr, idx) => {
        const inputs = tr.querySelectorAll('input, span[data-field]');
        ...
        inputs.forEach(inp => {
            ...
            else if (field === 'variance') prog.variance = val;
            else if (field === 'accomplishment') prog.accomplishment = val;
        });
...
function loadOfficeEditorReportTab(headEl, bodyEl, type, quarter, office) {
    ...
    .then(data => {
        ...
        if (office?.programs && type !== 'aip') {
            programs = programs.map(p => {
                const match = office.programs.find(m => m.code === p.code || m.name === p.name);
                if (match) {
                    const hasAip = p.aipAmount && (p.aipAmount.ps || p.aipAmount.mooe || p.aipAmount.co);
                    const hasBudget = p.annualBudget && (p.annualBudget.ps || p.annualBudget.mooe || p.annualBudget.co);
                    return {
                        ...p,
                        aipAmount: hasAip ? p.aipAmount : (match.aipAmount || p.aipAmount || { ps: 0, mooe: 0, co: 0 }),
                        annualBudget: hasBudget ? p.annualBudget : (match.annualBudget || p.annualBudget || { ps: 0, mooe: 0, co: 0, total: 0 }),
                    };
                }
                return p;
            });
        ...
        if (type === 'physical') {
            ...
            if (type === 'annual_financial') {
                applyQuarterlyObligationsToAdminTable(editingOfficeId, bodyEl);
            }
        }
    })
    .catch(() => {
